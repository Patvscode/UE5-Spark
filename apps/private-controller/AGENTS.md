...
Durable movement feedback: keep a persistent inline movement composer on the stage; directing a move must not require a modal. Commands run once by default, with explicit Loop and Stop controls. In `ai_motion` and `asset_aware_ai`, forward the user's complete movement description to ARDY's official text encoder without collapsing it into a catalog ID; apply only transport bounds such as text length, finite intensity, and finite duration. Keep the reviewed catalog solely for quick presets, deterministic mode, and fallback. The ordinary message field may route explicit movement requests before chat. Never claim that movement occurred unless the live ARDY/Unreal route accepted it.
...
Durable operations feedback: keep the lightweight private controller available after reboot and put one Start/Stop companion control inside the settings sheet for the project-owned ARDY and Unreal services. Fay remains externally managed. The same sheet should document the actual project directories, list available character/motion/AI/wardrobe options, and provide a narrow validated editor for persistent text configuration. Saves must be atomic with recoverable backups; user-created wardrobe profiles may be added or removed, while built-ins are deletion-protected. Clearly distinguish live controller settings from staged Unreal rebuild drafts, and never present binary `.uasset` content as browser-editable text.
