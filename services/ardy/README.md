...
`POST /v2/poses`. The incompatible v1 URL returns 404 rather than serving a v2
envelope under a misleading path. Requests accept only a reviewed behavior,
bounded intensity/duration, and the last consumed sequence. Arbitrary prompts
and credentials are not part of the service API. Protocol v2 responses contain
at most eight Core27 frames at 20 FPS. Each frame contains local XYZW rotations,
global posed-joint positions, root translation/rotation, and contacts. Neck and
head rotations are replaced with identity so StreamingADA retains ownership.
The root fields are a redundant stream anchor: root XYZ must equal global
`positions[0]` (Hips), and root rotation must equal local `joints[0]` in the
same quaternion hemisphere. Protocol validation rejects inconsistent data.
...
- `mock` produces deterministic protocol data for Unreal bridge and failure
  tests.
- `ardy` loads the approved Horizon8 checkpoint and only the nine reviewed
  cached embeddings in [`config/motion-catalog.json`](../../config/motion-catalog.json):
  `idle`, `listen`, `explain`, `wave`, `jog_in_place`, `run_in_place`,
  `jumping_jacks`, `stretch`, and `dance_relaxed`. It reports degraded health
  and returns 503 when the complete private cache is absent.
...
read-only and write only approved checkpoint IDs or the nine reviewed text
